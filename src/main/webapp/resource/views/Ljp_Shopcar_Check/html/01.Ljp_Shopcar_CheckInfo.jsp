...
<head>
	...
	%>
		...
		<link rel="stylesheet" type="text/css" href="<%=basePath%>/resource/views/Ljp_Shopcar_Check/css/01.Ljp_Shopcar_CheckInfo.css" />
			...
			<div class="infor">
				...
				<div class="user_info " >
					...
					<div class="use_newInfo">
						...
						<div class='yincang'>&nbsp;&nbsp;&nbsp;<input class='radio' type='radio' name="xuanze" id="xuanze" /> <label for="xuanze">使用新地址</label><br /></div>
							
					

						<div class="display">
							<span class="fie"><span class="field"><span>*</span> 收货人：</span><input class="user" type="text" /><br /></span>
							<span class="field"><span>*</span> 所在地区：</span><input name="" id="sel1" type="text" class="city_input" readonly="readonly" value="北京-北京市"><br />
							<span class="field"><span>*</span> 详细地址：</span><span class="sele_addr"></span><input class="area" type="text" /><br />
				...
				<div class="pay_way1">
					...
					<span>
						<p style="display: inline;"><input type="radio" name="f" id="s" value="" class="zffs" /><label for="s">在线支付</label>&nbsp;&nbsp;&nbsp;</p>
					<p style="display: inline;"><input type="radio" name="f" id="d" value=""  class="zffs"/><label for="d">货到付款</label>(选择该方式后不支持货到付款的商品仍需在线支付)<br /></p>
				...
				<div class="goods_list">
					...
					<div class="finsh" style="position: relative;">
						...
						<span class="money1">应付金额：<span class="total_prices" style="color: red;font-size: 16px;font-weight: bolder;">399</span>元</span>
						<a class="tijiao" style="position: absolute;cursor:pointer;right:20px;top:5px;width:100px;height:40px;" href="<%=basePath%>/resource/views/zhifu/html/lkl_zhifu.html"><button style="background: #FC6E75;color:white;text-align:center;line-height:20px;width:100px;height:40px;">提交订单</button></a>
...
//	console.log(data);
                for(var i = 0; i < data.length; i++) {
                    $("<p class='address'><input type='radio' checked='checked' class='xuanze' name='xuanze' id='xuanze" + i + "' value='" + data[i].xId + "' /><label for='xuanze" + i + "'><span class='xm'>" + data[i].shouhuoName + "</span>&nbsp;&nbsp;&nbsp;<span class='dh'>" + data[i].shouhuoPhone + "</span>&nbsp;&nbsp;&nbsp;" +
...
//	需要的参数
            ...
            var arrcs = [];
            $(".img_wid").each(function(index, el) {
                arrcs.push(el.innerHTML);
            })
            //	支付方式
            $(".zffs:checked").siblings("label").html();
            console.log($(".zffs:checked").siblings("label").html());
            //	总金额:
//	$(".total_prices").html($(".total_price").html());
            console.log($(".total_prices").html());
            //用户id
            //是否要发票
            $(".fapiao:checked").val();
            console.log($(".fapiao:checked").val());
            //留言
            $(".text_area").val();
            console.log($(".text_area").val());
            //评分
            $(".jifen").html();
            console.log($(".jifen").html())
            //运费
            $(".shouhuoren").html();
            $(".iphone").html();
            $(".dizhi").html();
            console.log($(".iphone").html());
            console.log($(".dizhi").html());
//	**************************************
            $.ajax({
